...
officeos bus get-cron-log <agent>
...
officeos bus migrate-crons <agent>

# Re-run for all enabled agents
officeos bus migrate-crons

# Bypass the marker and re-run regardless
officeos bus migrate-crons <agent> --force
```
...
The repo-shipped templates were swept clean in `0ccb3c98`, `b10711a4`, and `836f4759`. End-user agent workspaces created before that sweep need their own pass. The `officeos bus upgrade-cron-teaching` command exists to find and (where safe) fix those leftovers.
...
|---|---|---|
| `\bCronCreate\b` | Bare references to the `CronCreate` tool | `CronCreate` is for one-shot in-session reminders only. Persistent crons must use `officeos bus add-cron`. |
...
| `(configured in config.json)` | Stale parenthetical about cron source-of-truth | Cron source of truth is now `crons.json`, written by `officeos bus add-cron`. |
...
| Old | New |
|---|---|
| `Use CronCreate to schedule a 4h heartbeat` | `officeos bus add-cron $CTX_AGENT_NAME heartbeat 4h '<prompt>'` |
| `/loop 4h heartbeat` | `officeos bus add-cron $CTX_AGENT_NAME heartbeat 4h heartbeat` |
| `/loop create cron name=heartbeat ...` | `officeos bus add-cron <agent> <name> <interval> <prompt>` |
| `(configured in config.json)` | `(configured via officeos bus add-cron)` |
| `Edit config.json to add the cron entry` | `Run officeos bus add-cron <agent> <name> <interval> <prompt>` |

### Running the Scanner

```bash
# Scan one agent (exits 1 if any matches remain)
officeos bus upgrade-cron-teaching <agent>

# Scan every enabled agent under orgs/*/agents/*
officeos bus upgrade-cron-teaching

# Machine-readable output for scripting / CI
officeos bus upgrade-cron-teaching <agent> --json

# Apply the safe literal substitutions in-place
officeos bus upgrade-cron-teaching <agent> --apply
```
...
**Cron not firing:**
1. `officeos bus list-crons <agent>` — confirm the cron is registered and has a `next_fire_at`
2. `officeos bus get-cron-log <agent>` — check execution history for `status: retried` or `status: failed`
3. If PTY injection is failing, check the daemon log: `~/.cortextos/$CTX_INSTANCE_ID/logs/<agent>/`

**Migration did not run:**
- No marker file? Run `officeos bus migrate-crons <agent>` manually.
...
- If both the primary file and the `.bak` are unparseable, `readCrons()` returns `[]` and the scheduler starts with an empty schedule. Fix: write a valid `crons.json` using `officeos bus add-cron`, or restore from a known-good backup.
...
- New crons should be added via `officeos bus add-cron`, not edited in `config.json` directly.
- Syntax: `officeos bus add-cron <agent> <name> <interval|cron-expr> <prompt...>`
