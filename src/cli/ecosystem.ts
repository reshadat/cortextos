...
export const ecosystemCommand = new Command('ecosystem')
  ...
  .action(async (options: { instance: string; org?: string; output: string }) => {
    ...
    // so `officeos ecosystem` works from outside ~/cortextos.
    ...
    if (agents.length === 0) {
      console.log('No agents found. Add agents first: officeos add-agent <name>');
      return;
    }
    ...
    // Without the second check, running `officeos ecosystem` before
    ...
    // `officeos ecosystem` after `npm install` to add it.
    ...
    // Now: `CTX_INSTANCE_ID=other pm2 restart officeos-daemon` just works.
    //
    // BUG-016 fix: bumped max_restarts from 10 to 50. PM2's max_restarts
    // controls how many times PM2 itself restarts officeos-daemon if it
    ...
    //
    // BUG-019 fix: emit a officeos-dashboard PM2 entry alongside the daemon
    ...
    const content = `// AUTO-GENERATED by \`officeos ecosystem\`. Do NOT edit by hand.
// Re-run \`officeos ecosystem\` to regenerate.
//
...
// pm2 restart officeos-daemon\` switches instances without regenerating.
module.exports = {
  apps: [
    {
      name: 'officeos-daemon',
