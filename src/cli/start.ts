...
export const startCommand = new Command('start')
  ...
  .description('Start the officeOs daemon and agents')
  .action(async (agent: string | undefined, options: { instance: string; foreground?: boolean }) => {
    ...
    if (!daemonRunning) {
      ...
      if (options.foreground) {
        ...
        console.log('Starting officeOs daemon in foreground...');
      ...
      if (commandExists('pm2')) {
        ...
        if (existsSync(ecosystemPath)) {
          ...
          try {
            ...
            console.error('  officeos ecosystem && pm2 start ecosystem.config.js');
          }
        }
      } else {
    ...
    if (agent) {
      ...
      const response = await ipc.send({ type: 'start-agent', agent, source: 'officeos start' });
      if (response.success) {
      ...
      const response = await ipc.send({ type: 'status', source: 'officeos start' });
      if (response.success) {
        const statuses = response.data as any[];
        if (statuses.length === 0) {
          console.log('No agents configured. Add one with: officeos add-agent <name>');
        } else {
