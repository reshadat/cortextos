...
```bash
officeos bus kb-ingest <vault-path>/01-Memory --org [ORG] --scope shared
```
...
```json
{
  ...
  "cron": "0 4 * * *",
  "prompt": "Ingest today's memory files into the KB: officeos bus kb-ingest <vault-path>/01-Memory --org [ORG] --scope shared. Log the result.",
  "type": "recurring"
}
```
