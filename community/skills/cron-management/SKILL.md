...
description: "Manage persistent recurring scheduled tasks via the officeos bus. Crons are stored in crons.json, loaded and dispatched by the daemon, and survive agent restarts. Use this skill for all cron CRUD: create, update, remove, list, test-fire, and inspect execution history. Never use CronCreate or /loop for persistent recurring work — those are session-only and die on restart."
...
```bash
officeos bus list-crons $CTX_AGENT_NAME --json
```
...
```bash
officeos bus add-cron <agent> <name> <interval> <prompt...>
```
...
officeos bus add-cron $CTX_AGENT_NAME daily-sweep 1d Run the full daily workflow.
```
...
```bash
officeos bus add-cron $CTX_AGENT_NAME heartbeat 6h --desc "Agent liveness heartbeat" Read HEARTBEAT.md and follow its instructions.
```
...
officeos bus update-cron $CTX_AGENT_NAME heartbeat --cron-expr "0 */4 * * *"

# --cron-expr is an alias for --interval; both accept interval strings or 5-field expressions
officeos bus update-cron $CTX_AGENT_NAME morning-report --interval "0 8 * * 1-5"

# Update the prompt
officeos bus update-cron $CTX_AGENT_NAME heartbeat --prompt "Read HEARTBEAT.md, follow instructions, then log state."
...
officeos bus update-cron $CTX_AGENT_NAME heartbeat --interval 8h --prompt "Read HEARTBEAT.md and follow its instructions."
```
...
Example:
```bash
officeos bus remove-cron $CTX_AGENT_NAME heartbeat
```
...
```bash
officeos bus test-cron-fire $CTX_AGENT_NAME <name>
```
...
**Filter by cron name:**
```bash
officeos bus get-cron-log $CTX_AGENT_NAME heartbeat
```

**Limit entries (default 50):**
```bash
officeos bus get-cron-log $CTX_AGENT_NAME heartbeat --limit 20
```

**Machine-readable JSON:**
```bash
officeos bus get-cron-log $CTX_AGENT_NAME heartbeat --json
```
...
3. If `next_fire_at` is stale, the daemon may not have reloaded. Restart the agent or run `officeos bus migrate-crons $CTX_AGENT_NAME --force`.
...
- If both files are bad, restore via `add-cron` or re-migrate: `officeos bus migrate-crons $CTX_AGENT_NAME --force`.
...
```bash
officeos bus test-cron-fire $CTX_AGENT_NAME morning-report
# Watch agent PTY — the prompt should inject immediately
officeos bus get-cron-log $CTX_AGENT_NAME morning-report --limit 1
```
...
officeos bus get-cron-log $CTX_AGENT_NAME morning-report --limit 10
