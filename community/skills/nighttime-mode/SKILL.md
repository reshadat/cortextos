...
```
1. CHECK: officeos bus list-tasks --status in_progress
   → Any overnight tasks dispatched?

2. IF tasks are running:
   a. Check agent heartbeats: officeos bus read-all-heartbeats
   b. Check inbox for completion reports: officeos bus check-inbox
   c. Process completions, dispatch next tasks if queue has more
   d. GOTO step 1

3. IF no tasks pending:
   a. Begin preparing morning briefing data
   b. Update heartbeat: officeos bus update-heartbeat "preparing morning briefing"
```
...
```bash
officeos bus list-tasks --status in_progress
...
officeos bus complete-task "$TASK_ID" --result "<what was produced>"

# 2. Log the event
officeos bus log-event task task_completed info --meta '{"task_id":"'$TASK_ID'","agent":"<completing_agent>"}'
...
officeos bus list-tasks --status pending
```
...
officeos bus send-message <agent> normal '<unblocking info or reassignment>'
...
```bash
officeos bus update-heartbeat "nighttime mode - X/Y tasks complete, monitoring agents"
```
...
officeos bus update-heartbeat "morning briefing data ready - overnight complete"
```
...
officeos bus log-event action nighttime_mode_start info --meta '{"agent":"'$CTX_AGENT_NAME'"}'

# Task completions
officeos bus log-event task task_completed info --meta '{"task_id":"<id>","agent":"<completing_agent>"}'

# Morning ready
officeos bus log-event action morning_briefing_ready info --meta '{"tasks_completed":"X","tasks_blocked":"Y"}'
```
