...
- Weekly cron (configured via `officeos bus add-cron`)
...
# Filter by type
RESULT=$(officeos bus browse-catalog --type skill)
...
officeos bus install-community-item <item-name>
```
...
- The catalog is fetched during upstream sync (`officeos bus check-upstream`) and lives at `community/catalog.json` in the framework root
