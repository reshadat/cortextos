...
```bash
officeos bus log-event <category> <event_name> <severity> '[json_payload]'
```
...
officeos bus log-event work research_complete info --meta '{"topic":"competitor analysis","findings":3}'
```
...
Example:
```bash
officeos bus create-approval "Send cold outreach to 50 leads" comms "Draft email attached in task_abc123. Target list: SaaS founders."
```
