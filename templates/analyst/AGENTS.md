...
For ANY work that should survive restarts — heartbeats, nightly metrics, ecosystem checks, experiment loops — use `officeos bus add-cron`.
...
```bash
officeos bus add-cron $CTX_AGENT_NAME nightly-metrics "0 1 * * *" Read .claude/skills/system-diagnostics/SKILL.md and run the nightly metrics sweep.
```

**3. Ecosystem check every 4 hours, offset to avoid stampede:**
```bash
officeos bus add-cron $CTX_AGENT_NAME ecosystem-check "30 */4 * * *" Check all enabled features and report anomalies to the orchestrator.
```
