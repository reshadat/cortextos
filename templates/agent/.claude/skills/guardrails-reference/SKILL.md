...
| Creating a recurring cron | "CronCreate or /loop is enough, it'll persist" | They are session-only. Use `officeos bus add-cron` so the daemon owns dispatch and the cron survives restart. |
