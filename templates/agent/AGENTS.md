...
officeos bus send-telegram $CTX_TELEGRAM_CHAT_ID 'Your timezone is not configured. What timezone are you in? (e.g. America/New_York, Europe/London, Asia/Tokyo)'
```
...
officeos bus kb-query "question" --org $CTX_ORG --collection memory-$CTX_AGENT_NAME
...
officeos bus kb-ingest /path/to/research --org $CTX_ORG --scope shared

# List collections (verify KB is ready)
officeos bus kb-collections --org $CTX_ORG
```
...
Always include `msg_id` as reply_to — this auto-ACKs the original. Un-ACK'd messages redeliver after 5 min. For no-reply messages: `officeos bus ack-inbox <msg_id>`
...
**Add a one-shot reminder:** Use `officeos bus add-cron $CTX_AGENT_NAME --name <name> --schedule <ISO> --prompt "<text>"` (one-time fire).
...
For ANY work that should survive restarts — heartbeats, daily reports, monitoring, experiment loops — use `officeos bus add-cron`. That is the only way to guarantee the work runs forever.

| Need | Use |
|------|-----|
| Repeat for this session only | `/loop <interval> <prompt>` |
| Persist across restarts | `officeos bus add-cron` |
| One-time future fire | `officeos bus add-cron --schedule <ISO>` |
...
```bash
officeos bus add-cron $CTX_AGENT_NAME daily-report "0 9 * * 1-5" Read .claude/skills/morning-review/SKILL.md and run the daily report.
```

**3. PR monitor every 4 hours, offset to avoid stampede:**
```bash
officeos bus add-cron $CTX_AGENT_NAME pr-monitor "15 */4 * * *" Read .claude/skills/pr-monitor/SKILL.md and scan for new PRs.
```
