...
| Error occurs | "It's minor, I'll keep going" | Log the error via `officeos bus log-event`. Report it. Silent failures are invisible failures. |
...
| Creating a recurring cron | "An in-session scheduler is enough, it'll persist" | Session-local schedulers die on restart. Always use `officeos bus add-cron` so the daemon owns dispatch and the cron survives every kind of restart. |
