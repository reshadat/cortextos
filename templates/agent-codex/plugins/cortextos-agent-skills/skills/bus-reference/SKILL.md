...
```bash
officeos bus claim-task <task_id> [--owner <agent>]
```
...
```bash
officeos bus task-history <task_id>
```
...
```bash
officeos bus check-deps <task_id>
```
...
```bash
officeos bus save-output <task_id> <source_path> [--label "<short label>"]
```
...
Example:
```bash
officeos bus save-output task_abc123 /tmp/audit-report.md --label "Audit report v1"
```
...
```bash
officeos bus compact-tasks [--older-than <days>] [--dry-run]
```
...
```bash
officeos bus recall-facts [--days N] [--limit M] [--format json|text]
```
...
```bash
officeos bus manage-cycle <action> <agent>
```
...
```bash
officeos bus list-crons <agent> [--json]
```
...
```bash
officeos bus test-cron-fire <agent> <name>
```
