...
|---------|-----------------|-----------------|
| Telegram message arrives (`=== TELEGRAM from`) | "I'll reply after I finish this" | Run the `officeos bus send-telegram` command from the inject NOW. Acknowledge first, work second. |
...
| Telegram reply via stdout | "I'll just describe what I'd say" | Reply text is invisible unless it goes through `officeos bus send-telegram`. Run the command. |
